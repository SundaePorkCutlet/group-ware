...
export default function WorkSummarySidebar({
  refreshTrigger = 0,
}: WorkSummarySidebarProps) {
  const user = useAuthStore((state) => state.user);
  const profile = useAuthStore((state) => state.profile);
  const { myEvents } = useEventStore();

  console.log("🔍 WorkSummarySidebar - authStore 데이터:", { user, profile });
  ...
  // 유저 프로필 정보 fetch (authStore 우선, 없으면 DB에서 가져오기)
  useEffect(() => {
    if (!user) return;

    console.log("🔍 WorkSummarySidebar - 프로필 데이터 처리 시작:", {
      userId: user.id,
      hasProfile: !!profile,
      profileData: profile,
    });

    // authStore에 프로필 데이터가 있으면 우선 사용
    if (profile && profile.id === user.id) {
      console.log("🔍 WorkSummarySidebar - authStore 프로필 사용:", profile);
      const weeklyHours = Number(profile.weekly_work_hours ?? 40);
      const workStartDay = String(profile.weekly_work_start ?? "월");
      const workEndDay = String(profile.weekly_work_end ?? "금");
      const annualLeaveDays = Number(profile.annual_leave ?? 15);

      console.log("🔍 WorkSummarySidebar - authStore 프로필 데이터 설정:", {
        weeklyHours,
        workStartDay,
        workEndDay,
        annualLeaveDays,
      ...
      setWeeklyWorkHours(weeklyHours);
      setWorkStart(workStartDay);
      setWorkEnd(workEndDay);
      setAnnualLeave(annualLeaveDays);
      return;
    }

    // authStore에 없거나 사용자 ID가 다르면 DB에서 가져오기
    console.log(
      "🔍 WorkSummarySidebar - DB에서 프로필 데이터 가져오기 시작:",
      user.id
    );
    ...
    (async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "weekly_work_hours, weekly_work_start, weekly_work_end, annual_leave"
        )
        .eq("id", user.id)
        .single();

      console.log("🔍 WorkSummarySidebar - 프로필 쿼리 결과:", { data, error });

      if (!error && data) {
        const weeklyHours = Number(data.weekly_work_hours ?? 40);
        const workStartDay = String(data.weekly_work_start ?? "월");
        const workEndDay = String(data.weekly_work_end ?? "금");
        const annualLeaveDays = Number(data.annual_leave ?? 15);

        console.log("🔍 WorkSummarySidebar - DB 프로필 데이터 설정:", {
          weeklyHours,
          workStartDay,
          workEndDay,
          annualLeaveDays,
        ...
        console.error(
          "🔍 WorkSummarySidebar - 프로필 데이터 가져오기 실패:",
          error
        );
      }
    })();
  ...
  // eventStore에서 이벤트 데이터 가져오기
  useEffect(() => {
    if (!user || !myEvents.length) return;

    console.log("🔍 WorkSummarySidebar - myEvents:", myEvents.length);
    console.log("🔍 WorkSummarySidebar - myEvents 샘플:", myEvents.slice(0, 3));
    ...
    console.log("🔍 WorkSummarySidebar - 이번 주 범위:", {
    ...
    console.log(
      "🔍 WorkSummarySidebar - 출퇴근 이벤트:",
      attendanceData.length
    );
    setAttendanceEvents(attendanceData as unknown as AttendanceEvent[]);

    // myEvents에서 휴가 이벤트 필터링 (올해 전체)
    const yearStart = new Date(now.getFullYear(), 0, 1);
    const yearEnd = new Date(now.getFullYear(), 11, 31, 23, 59, 59, 999);
    const holidayData = myEvents.filter(
      (event) =>
        event.event_type === "holiday" &&
        event.leave_type &&
        new Date(event.start_date) >= yearStart &&
        new Date(event.end_date) <= yearEnd
    );
    console.log("🔍 WorkSummarySidebar - 휴가 이벤트:", holidayData.length);
    setHolidayEvents(holidayData as unknown as HolidayEvent[]);
  }, [user, workStart, workEnd, myEvents, refreshTrigger]);

  // 누적 근무시간 계산 (events 테이블만 사용, 각 날짜별 점심시간 제외 설정 적용)
  useEffect(() => {
    console.log("🔍 WorkSummarySidebar - 근무시간 계산 시작");
    console.log(
      "🔍 WorkSummarySidebar - attendanceEvents:",
      attendanceEvents.length
    );
    console.log("🔍 WorkSummarySidebar - holidayEvents:", holidayEvents.length);
    ...
    console.log("🔍 WorkSummarySidebar - eventsByDate:", eventsByDate);

    // 날짜별로 출근/퇴근 시간 계산
    Object.values(eventsByDate).forEach((dayData) => {
      if (dayData.clockIn && dayData.clockOut) {
        const diff =
          (dayData.clockOut.getTime() - dayData.clockIn.getTime()) / 60000; // 분 단위
        if (diff > 0) {
          // 해당 날짜의 점심시간 제외 설정에 따라 계산
          const workTime = dayData.excludeLunch ? Math.max(diff - 60, 0) : diff;
          total += workTime;
          console.log("🔍 WorkSummarySidebar - 일일 근무시간:", {
            date: dayData.clockIn?.toISOString().slice(0, 10),
            diff,
            workTime,
            total,
          });
    ...
    const weeklyHolidays = holidayEvents.filter((event) => {
      ...
      return (
        ...
        start <= weekEnd &&
        ["1", "2", "3"].includes(String(event.leave_type))
      );
    });
    weeklyHolidays.forEach((event) => {
      ...
      const end =
        new Date(event.end_date) > weekEnd ? weekEnd : new Date(event.end_date);
    ...
    console.log("🔍 WorkSummarySidebar - 최종 근무시간:", {
      total,
      holidayTotalMin,
      weeklyWorkHours,
    });
  ...
  // 올해 사용한 연차 계산
  useEffect(() => {
    console.log("🔍 WorkSummarySidebar - 연차 계산 시작");
    console.log("🔍 WorkSummarySidebar - holidayEvents:", holidayEvents.length);
    ...
    holidayEvents
      ...
      .forEach((event) => {
        ...
        console.log("🔍 WorkSummarySidebar - 연차 계산:", {
    ...
    console.log("🔍 WorkSummarySidebar - 최종 사용 연차:", used);
  ...
  const calculateTodayLeaveTime = () => {
    ...
    const actualWorkedToday = excludeLunch
      ? Math.max(workedToday - 60, 0)
      : workedToday;
    ...
    attendanceEvents.forEach((event) => {
      ...
      if (event.title === "🌅 출근") {
        ...
        const clockOut = attendanceEvents.find(
          (e) =>
            e.title === "🌆 퇴근" &&
        ...
        if (clockOut) {
          const diff =
            (new Date(clockOut.start_date).getTime() - clockIn.getTime()) /
            60000;
    ...
    const todayRemaining = Math.max(
      weeklyWorkHours * 60 - otherDaysWorked - actualWorkedToday,
      0
    );
  ...
  return (
    <aside className="bg-white rounded-xl shadow p-4 sm:p-6 flex flex-col gap-4 border">
      {/* 모바일에서는 가로 배치, 데스크톱에서는 세로 배치 */}
      <div className="lg:block">
        ...
        <div className="hidden lg:flex flex-col gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-blue-700 ml-1">
              {weeklyWorkHours}시간
            </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-blue-700 ml-1">
              {(annualLeave - usedAnnualLeave).toFixed(2)}일
            </span>
            <span className="text-sm text-gray-500 ml-1">
              / {annualLeave}일
            </span>
        ...
        <div className="lg:hidden">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <CalendarDays className="w-4 h-4 text-blue-600" />
              <span className="text-sm font-bold text-gray-900">근무 요약</span>
            </div>
            <button
              onClick={() => setShowSetting(true)}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <div className="text-xs text-gray-500 mb-1">주 근무시간</div>
            ...
            <div className="text-center">
              <div className="text-xs text-gray-500 mb-1">남은 연차</div>
              <div className="text-lg font-bold text-green-700">
                {(annualLeave - usedAnnualLeave).toFixed(1)}일
              </div>
              <div className="text-xs text-gray-400">/ {annualLeave}일</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-semibold text-indigo-700 ml-1 text-sm lg:text-base">
              {formatHourMin(totalWorked)}
            </span>
          </div>
          {/* 근무시간 ProgressBar */}
          <ProgressBar
            percent={Math.round((totalWorked / (weeklyWorkHours * 60)) * 100)}
            color="bg-indigo-500"
          />
        </div>
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-semibold text-rose-700 ml-1 text-sm lg:text-base">
              {formatHourMin(remainMin)}
            </span>
          </div>
          {/* 남은 근무 ProgressBar (반대 방향) */}
          <ProgressBar
            percent={Math.round((remainMin / (weeklyWorkHours * 60)) * 100)}
            color="bg-rose-500"
          />
        </div>
      </div>
      ...
      {/* 오늘 퇴근 가능 시간 표시 - 모바일에서 더 작게 */}
      {todayLeaveInfo && (
        <div className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg p-3 lg:p-4 mt-4">
          <div className="flex items-center gap-2 mb-2">
            <Clock className="w-4 h-4 lg:w-5 lg:h-5 text-green-600" />
            <span className="font-bold text-green-800 text-sm lg:text-base">
              오늘 퇴근 가능 시간
            </span>
          </div>
          <div className="text-center">
            ...
            {holidayEvents
              ...
              .map((event) => {
                ...
                return (
                  <li
                    key={event.id}
                    className="flex items-center gap-2 text-sm border-b last:border-b-0 py-1"
                  >
                    <span className="text-gray-700 font-mono min-w-[110px]">
                      {dateStr}
                    </span>
