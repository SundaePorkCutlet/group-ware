...
export default function Calendar({
  ...
  const getEventsForDate = (date: Date) => {
    return events.filter((event) => {
      // 캘린더 토글 상태에 따라 필터링
      if (event.visibility === "personal" && !showPersonalCalendar)
        return false;
  ...
  const getTransactionsForDate = (date: Date) => {
    ...
    return transactions.filter((tx) => {
      ...
      // 디버깅 로그
      if (tx.date === "2025-01-01") {
        console.log(
          `거래 내역 필터링: ${tx.date} vs ${
            date.toISOString().split("T")[0]
          } = ${match}`
        );
        console.log(
          `txDate: ${txDate.toISOString()}, date: ${date.toISOString()}`
        );
      }
                ...
                <div className="space-y-0.5 sm:space-y-1">
                  {(() => {
                    ...
                    if (attendanceEvents.length > 0) {
                      const clockIn = attendanceEvents.find(
                        (e) => e.title === "🌅 출근"
                      );
                      ...
                      const combinedEvent = {
                        ...
                        description: "출퇴근 기록",
                        start_date:
                          clockIn?.start_date || clockOut?.start_date || "",
                        end_date: clockOut?.end_date || clockIn?.end_date || "",
                        location: "",
                        created_by:
                          clockIn?.created_by || clockOut?.created_by || "",
                        department_id: "",
                        is_all_day: false,
                        event_type: "attendance" as const,
                        visibility: "personal" as const,
                        created_at:
                          clockIn?.created_at || clockOut?.created_at || "",
                        updated_at:
                          clockIn?.updated_at || clockOut?.updated_at || "",
                      ...
                      if (clockIn && clockOut) {
                        combinedEvent.title = `${formatTime(
                          clockIn.start_date
                        )}-${formatTime(clockOut.start_date)}`;
                      } else if (clockIn) {
                        combinedEvent.title = `${formatTime(
                          clockIn.start_date
                        )} 출근`;
                      ...
                      return (
                        <span
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onShowAllEventsForDate &&
                              onShowAllEventsForDate(date, dayEvents);
                          }}
                        >
